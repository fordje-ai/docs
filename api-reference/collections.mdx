...
Collections are saved configurations of data point types and trade groups that can be executed against any set of AHJs on demand. Collections are managed through the Fordje platform and accessed read-only through the API.
...
Executes the collection's saved filters against specified AHJs, returning matching data point values. AHJ access is enforced — unauthorized AHJs are excluded with a warning.
