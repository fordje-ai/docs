...
- **Category IDs** — the trades or disciplines the data points belong to (e.g., electrical, plumbing, solar)
...
The `data_point_type_ids` and `category_ids` fields tell you exactly what filters the collection applies. You can cross-reference these with the [reference data endpoints](/api-reference/introduction) to see their human-readable names.
...
See the [Collections API reference](/api-reference/introduction) for the complete endpoint documentation, including all query parameters, response fields, and pagination details.
