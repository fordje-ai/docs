...
When you need to compare regulatory requirements across multiple jurisdictions at once, reports give you a structured, side-by-side view. Instead of querying data points one AHJ at a time and stitching results together yourself, a report generates a comparison matrix in a single request — AHJs on one axis, data point types on the other.
...
- **Compliance comparison.** Your engineering team needs to understand how building codes differ between jurisdictions for a specific regulatory area. Compare insulation R-values, fire sprinkler thresholds, or permit requirements across every county in a state to identify where your standard designs need modification.

<Tip>
  If you find yourself running the same comparison repeatedly with the same set
  of data point types, consider creating a [collection](/concepts/collections)
  instead. Collections save the data point type configuration so you only need
  to pass AHJ IDs each time.
</Tip>

## The matrix format

The matrix report organizes results into rows, one per AHJ. Each row contains a `values` object where the keys are data point type IDs and the values are published value objects for that jurisdiction.
...
In this example, data point type `10` might be "Permit Required", `20` might be "Inspection Frequency", and `30` might be "Permit Fee". You can look up the human-readable names for any data point type ID using `GET /v1/data-points/types`. See the [data points concept page](/concepts/data-points) for more on types and values, or use the copy-ready ID arrays in the [standard solar data point types](/concepts/standard-solar-data-point-types).

<Info>
  If a jurisdiction does not have a value for a particular data point type, that
  cell is returned as `null`. Your code should handle `null` cells rather than
  assuming every data point type has a value for every AHJ.
</Info>
...
<Tip>
  CSV exports are a good fit for distributing requirement summaries to team
  members who do not interact with the API directly. Generate the report
  programmatically on a schedule and share the file through your existing
...
<CardGroup cols={2}>
  <Card
    title="Matrix report"
    icon="table-cells"
    href="/api-reference/reports/generate-matrix-comparison-report"
  >
    Full API reference for `POST /v1/reports/matrix` — request schema and
    response format.
  </Card>
  <Card
    title="CSV export"
    icon="file-csv"
    href="/api-reference/reports/export-report-as-csv"
  >
    Full API reference for `POST /v1/reports/export` — export comparisons as
    downloadable CSV.
  </Card>
</CardGroup>
