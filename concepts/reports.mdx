...
The matrix report organizes results into rows, one per AHJ. Each row contains a `values` object where the keys are data point type IDs and the values are current approved value objects for that jurisdiction.
...
<Info>
  If a jurisdiction does not have a value for a particular data point type, that cell is returned as `null`. Your code should handle `null` cells rather than assuming every data point type has a value for every AHJ.
</Info>
...
<CardGroup cols={2}>
  <Card title="Matrix report" icon="table-cells" href="/api-reference/endpoint/create_matrix_report">
    Full API reference for `POST /v1/reports/matrix` — request schema and response format.
  </Card>
  <Card title="CSV export" icon="file-csv" href="/api-reference/endpoint/export_report">
    Full API reference for `POST /v1/reports/export` — export comparisons as downloadable CSV.
  </Card>
</CardGroup>
