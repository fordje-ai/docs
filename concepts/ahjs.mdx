...
The detail response includes geographic and demographic fields like `county`, `population`, `lat`/`lon`, and `census_geoid` that are not present in the list view. The `census_geoid` is the Census Bureau GEOID (FIPS code) for the jurisdiction, useful for joining with Census data products.

{/* TODO: Verify census_geoid is consistently returned from backend AHJ detail endpoint. The backend model has the field but the Connect typed schema does not yet include it — the raw backend response is passed through. */}
...
<CardGroup cols={2}>
  ...
  <Card title="Get AHJ detail" icon="magnifying-glass" href="/api-reference/endpoint/get_ahj">
